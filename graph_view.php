...
switch ($_REQUEST['action']) {
	...
	case 'preview':
				...
				<form style='margin:0px;padding:0px;' name='form_graph_view' method='post' onSubmit='applyGraphPreviewFilterChange(document.form_graph_view);
					...
					<table cellpadding='1' cellspacing='0'>
						<tr class='noprint'>
									...
									if (sizeof($hosts) > 0) {
										foreach ($hosts as $host) {
											...
											if (get_request_var_request('host_id') == $host['id']) {
												print ' selected';
											} print '>' . htmlspecialchars($host['name']) . "</option>\n";
										}
									}
									?>
								</select>
								...
								<select id='graph_template_id' name='graph_template_id' onChange='applyGraphPreviewFilterChange(document.form_graph_view)'>
									...
									if (sizeof($graph_templates) > 0) {
										foreach ($graph_templates as $template) {
											...
											if (get_request_var_request('graph_template_id') == $template['id']) {
												print ' selected';
											} print '>' . htmlspecialchars($template['name']) . "</option>\n";
										}
									}
									?>
								</select>
							...
							<td>
								<input id='detail' type='checkbox' name='detail' onClick='applyGraphPreviewFilterChange(document.form_graph_view)' <?php print ((get_request_var_request("detail") == 'true') ? 'checked' : ''); ?>>
							</td>
							<td>
								Search:
							</td>
		...
		$graphs = db_fetch_assoc($graphQuery);
